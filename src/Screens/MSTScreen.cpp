...
MSTScreen::MSTScreen() 
    : homeBtn(20, 20, 130, 75, "Home", IconType::NONE),
      edgeInput(170, 20, 160, 75, "u v w", 15, false),
      addBtn(340, 20, 100, 75, "Add", IconType::NONE),
      delBtn(450, 20, 100, 75, "Del", IconType::NONE),
      oldNodeInput(560, 20, 100, 75, "Old", 10, true),
      newNodeInput(670, 20, 100, 75, "New", 10, true),
      updateBtn(780, 20, 130, 75, "Update", IconType::NONE),
      runBtn(920, 20, 150, 75, "Run MST", IconType::NONE),
      
      arrayInput(170, 110, 400, 75, "1 2 10, 2 3 20", 60, false),
      initBtn(580, 110, 120, 75, "Init", IconType::NONE),
      randomBtn(710, 110, 120, 75, "Random", IconType::NONE), // ĐÃ THÊM LẠI NÚT RANDOM
      clearBtn(840, 110, 120, 75, "Clear", IconType::NONE),   // DỊCH CLEAR SANG PHẢI
...
void MSTScreen::Update(AppState& currentState) {
    ...
    edgeInput.Update(); oldNodeInput.Update(); newNodeInput.Update(); arrayInput.Update(); graph.Update();
    if (homeBtn.Update()) currentState = AppState::MAIN_MENU;

    if (addBtn.Update()) {
        std::stringstream ss(edgeInput.GetText()); int u, v, w;
        if (ss >> u) { if (ss >> v && ss >> w) graph.AddEdge(u, v, w); else graph.AddVertex(u); edgeInput.Clear(); }
    }
    ...
    if (runBtn.Update()) { graph.RunKruskal(); }
    
    if (initBtn.Update()) {
        std::string text = arrayInput.GetText(); std::vector<InitEdge> edgeList; std::vector<int> isolated;
        std::stringstream ss(text); std::string token;
        while (std::getline(ss, token, ',')) {
            std::stringstream ts(token); int u, v, w;
            if (ts >> u) { if (ts >> v && ts >> w) edgeList.push_back({u, v, w}); else isolated.push_back(u); }
        }
        if(!edgeList.empty() || !isolated.empty()){ graph.InitFromEdges(edgeList, isolated); arrayInput.Clear(); }
    }
    
    if (randomBtn.Update()) graph.InitRandom(6); // SỰ KIỆN NÚT RANDOM HOẠT ĐỘNG TRỞ LẠI
...
void MSTScreen::Draw(Theme theme, Font uiFont, Font monoFont) {
    float sh = GetScreenHeight();
    homeBtn.Draw(theme, uiFont); edgeInput.Draw(theme, uiFont); addBtn.Draw(theme, uiFont); delBtn.Draw(theme, uiFont);
    oldNodeInput.Draw(theme, uiFont); newNodeInput.Draw(theme, uiFont); updateBtn.Draw(theme, uiFont); runBtn.Draw(theme, uiFont);
    arrayInput.Draw(theme, uiFont); initBtn.Draw(theme, uiFont); 
    randomBtn.Draw(theme, uiFont); // VẼ NÚT RANDOM
