...
void TrieScreen::Update(AppState& currentState) {
    ...
    if (addBtn.Update()) {
        ...
        if (!valStr.empty()) { trie.Insert(valStr); inputBox.Clear(); }
    }
    
    if (delBtn.Update()) {
        std::string valStr = inputBox.GetText();
        if (!valStr.empty()) { trie.Delete(valStr); inputBox.Clear(); } 
    }

    if (searchBtn.Update()) {
        std::string valStr = inputBox.GetText();
        if (!valStr.empty()) { trie.Search(valStr); inputBox.Clear(); }
    }
    ...
    if (initArrayBtn.Update()) {
        ...
        std::string temp;
        trie.ClearList(); 
        while (ss >> temp) { trie.Insert(temp); }
        arrayInput.Clear();
    }
    ...
    if (modeBtn.Update()) {
        trie.isStepByStep = !trie.isStepByStep;
        if (trie.isStepByStep) modeBtn.SetText("Mode: Step");
        else modeBtn.SetText("Mode: Auto");
    }

    if (trie.isStepByStep) {
        if (prevBtn.Update()) trie.StepBackward();
        if (nextBtn.Update()) trie.StepForward();
    }

    float sliderX = 960.0f;
    Rectangle sliderHitbox = {sliderX, botY + 15, 200, 40}; 
...
void TrieScreen::Draw(Theme theme, Font uiFont, Font monoFont) {
    ...
    inputBox.Draw(theme, uiFont); addBtn.Draw(theme, uiFont);
    ...
    Rectangle sliderBar = {sliderX, botY + 35.0f, 200.0f, 8.0f};
    DrawRectangleRounded(sliderBar, 0.5f, 10, theme.panelBorder);
    
    float fillWidth = ((trie.speedMultiplier - 0.5f) / 4.5f) * 200.0f;
    Rectangle fillBar = {sliderX, botY + 35.0f, fillWidth, 8.0f};
    DrawRectangleRounded(fillBar, 0.5f, 10, theme.btnGradEnd);

    DrawCircle(sliderX + fillWidth, botY + 39.0f, 12.0f, theme.textMain); 
